...
% Manual spectral estimation using windowed FFT (no Signal Processing Toolbox needed)
w_han_ = 0.5*(1-cos(2*pi*(1:nx)'/(nx+1)));  % symmetric Hanning window
fs_    = 1/dt;
nf_    = floor(nx/2)+1;
Xr_    = fft(real(xres).*w_han_, nx);
Xi_    = fft(imag(xres).*w_han_, nx);
sc_    = sum(w_han_.^2)*fs_;
Pxr    = abs(Xr_(1:nf_)).^2/sc_;
Pxi    = abs(Xi_(1:nf_)).^2/sc_;
Pxc    = Xr_(1:nf_).*conj(Xi_(1:nf_))/sc_;
Pxr(2:end-1)=2*Pxr(2:end-1);  % one-sided doubling (matches pwelch convention)
Pxi(2:end-1)=2*Pxi(2:end-1);
Pxc(2:end-1)=2*Pxc(2:end-1);
fx     = (0:nf_-1)'*fs_/nx;
Pxr=Pxr/2/dt;
Pxi=Pxi/2/dt;
Pxc=Pxc/2/dt;
